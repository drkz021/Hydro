var express = require("express");
var router = express.Router();
const cookieParser = require('cookie-parser');

function requireLogin(req, res, next) {
  if (req.session && req.session.user) {
    return next();
  } else {
    res.redirect('/login');
  }
}
// Require authentication middleware
const requireAuth = (req, res, next) => {
    if (req.isAuthenticated()) {
      return next();
    }
    res.redirect('/login');
};

const  credential = {
    email : "admin.local",
    password : "admin123",
    email1: "raspberrypi.local",
}

const nodemailer = require('nodemailer');

// create reusable transporter object using SMTP transport
let transporter = nodemailer.createTransport({
    host: 'smtp.gmail.com',
    port: 465,
    secure: true,
    auth: {
        user: 'hydroponicsystem2023@gmail.com',
        pass: 'dbnpsvffdkcypuwy'
    }
});

var mysql = require('mysql');
//const { send } = require("process");

var con1 = mysql.createConnection({
    host: "localhost",
    user: "admin",
    password: "opticfiber01",
    database: "login"
});

con1.connect(function (err) {

});

// login user
router.get('/login', (req, res)=>{
  

    con1.query(`SELECT * FROM users`, function (err, result, fields) {
   



        let isLogin = result.find((res) => 
            res.email == req.query.email && res.password == req.query.password && res.code == 'verified'
        );

        let isNotVerified = result.find((res) => 
        res.email == req.query.email && res.password == req.query.password && res.code != 'verified'
    );
  
  
        if (isLogin != null ) {
                  req.session.user = req.query.email ;
            //res.send('Logged IN');
             res.redirect('/route/homepage');
            
        }
        else if (isNotVerified != null ) {
            req.session.user = req.query.email ;
      //res.send('Logged IN');
      res.redirect('/not_verified');
      
  }
        else{
            res.redirect('/error');
        }

        
    
});

});

router.get('/register', (req, res) => {
  res.render('register', {title: "Register"} );
});


router.get('/casa/login', (req, res) => {

    
    res.render('login', {title: "Login"} );
  });

  router.get('/forgotPass', (req, res) => {
    res.render('forgotPass', {title: "Forgot Password"} );
  });


  router.get('/sendSMTP', (req, res) => {

  //  res.send('sent');
    console.log(req.query.email);
 
    transporter.sendMail( {
        from: 'hydroponicsystem2023@gmail.com', // sender address
        to:  req.query.email.toString(), // list of receivers
        subject: 'Casa Selada verification', // Subject line
        text: 'Here is your password', // plain text body
      //  html: '<b>Hello world?</b>' // html body

    }, (error, info) => {
        if (error) {
            return console.log(error);
        }
       // console.log('Message sent: %s', info.messageId);
    },
    );

        res.redirect('/route/casa/login')
  });
router.get('/error', (req, res) => {
    res.render('error',{title: "Error Prompt"})
});


router.get('/not_verified', (req, res) => {
    res.render('not_verified',{title: "Not_verified Prompt"})
});
router.get('/verify', (req, res) => {

    res.render('error',{title: "Error Prompt"})
});

router.get('/LOGIN', (req, res) => {

    res.send('logged in')
})

// route for dashboard with middleware
router.get('/dashboard', (req, res) => {
    if(req.session.user){
        res.render('dashboard',{title: "Fiber Optic Dashboard"})
    }else{
        res.render('error');
    }
});

// route for homepage with middleware
router.get('/homepage', (req, res) => {
    if(req.session.user){
        res.render('homepage',{title: "Fiber Optic Dashboard"})
    }else{
        res.render('error');
    }
});

// route for control with middleware
router.get('/control', (req, res) => {
    if(req.session.user){
        res.render('control',{title: "Fiber Optic Dashboard"})
    }else{
        res.render('error');
    }
});

// route for aboutus with middleware
router.get('/aboutus', (req, res) => {
    if(req.session.user){
        res.render('aboutus',{title: "Fiber Optic Dashboard"})
    }else{
        res.render('error');
    }
});


// route for logout
router.get('/logout', (req ,res)=>{
    req.session.destroy(function(err){
        if(err){
            console.log(err);
            res.send("Error")
        }else{
            res.render('login', { title: "Express", logout : "logout Successfully...!"})
        }
    })
});

router.use(cookieParser())

// middleware route for homepage
router.get('/route/homepage', (req, res) => {
    if(req.session.user){
        res.render('homepage',{title: "Fiber Optic Dashboard"})
    }else{
        res.redirect('/login');
    }
});
router.get('/route/dashboard', (req, res) => {
    if(req.session.user){
        res.render('dashboard',{title: "Fiber Optic Dashboard"})
    }else{
        res.redirect('/login');
    }
});
router.get('/route/control', (req, res) => {
    if(req.session.user){
        res.render('control',{title: "Fiber Optic Dashboard"})
    }else{
        res.redirect('/login');
    }
});
router.get('/route/aboutus', (req, res) => {
    if(req.session.user){
        res.render('aboutus',{title: "Fiber Optic Dashboard"})
    }else{
        res.redirect('/login');
    }
});

module.exports = router;
