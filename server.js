...
io.sockets.on("connection", socket => {
    ...
    socket.on("aerator", (arg) => {
      ...
      if (arg == true) {
        ...
        const url = "http://192.168.1.157/cm?cmnd=Power%20On";
        ...
        io.sockets.emit("aero_status", arg);
        const url = "http://192.168.1.157/cm?cmnd=Power%20Off";
    ...
    socket.on('nav1', function (args) {
      ...
      socket.on('defaultGrowlights', function (args) {
        con.query(`UPDATE schedule SET time_on = "06:00", time_off = "20:00",monday = "1", tuesday = "1",wednesday = "1",thursday = "1",friday = "1",saturday = "1",sunday = "1", status = "1" WHERE schedule.id = 1;`, function (err, result, fields) {
        });
...
const renderTime = () => {
  ...
  //  //console.log(result);
    ...
    if (result[0]['status']== "1" && result[0][day[d.getDay()]] == 1 && hour == "06" && minutes == "00") {
      const url = "http://192.168.1.130/cm?cmnd=Power%20On";
      con.query(` UPDATE status SET light_status = 'ON' WHERE status.id = 1;`, function (err, result, fields) {
      });
      // console.log("Grow Light is on");
      request.get(url, (error, response, body) => {});
    }
    ...
    if (result[0]['status'] == "1" && result[0][day[d.getDay()]] == 1 && hour == "20" && minutes == "00") {
      const url = "http://192.168.1.130/cm?cmnd=Power%20Off";
      con.query(` UPDATE status SET light_status = 'OFF' WHERE status.id = 1;`, function (err, result, fields) {
      });
      // console.log("Grow Light is off");
      request.get(url, (error, response, body) => {});
    }
  });
...
app.get('/', (req, res) => {
  res.render('homepage', { title: "Casa Selada" });
});
...
app.get('/control', (req, res) => {
  if(req.session.user){
    res.render('control', { title: "Casa Selada" })
  }else{
      res.render('login');
  }
});
app.get('/aboutus', (req, res) => {
  if(req.session.user){
    res.render('aboutus', { title: "Casa Selada" })
  }else{
      res.render('login');
  }
});
